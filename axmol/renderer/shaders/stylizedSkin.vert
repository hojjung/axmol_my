...
void getSkinnedPositionAndNormal(out vec4 position, out vec3 normal)
{
    ...
    position = vec4(dot(sourcePosition, matrixRow0),
                    dot(sourcePosition, matrixRow1),
                    dot(sourcePosition, matrixRow2),
                    1.0);

    vec4 sourceNormal = vec4(a_normal, 0.0);
    normal = vec3(dot(sourceNormal, matrixRow0),
                  dot(sourceNormal, matrixRow1),
                  dot(sourceNormal, matrixRow2));
}
...
void main()
{
    vec4 position;
    vec3 normal;
    getSkinnedPositionAndNormal(position, normal);

    vec4 worldPosition = u_MVMatrix * position;
